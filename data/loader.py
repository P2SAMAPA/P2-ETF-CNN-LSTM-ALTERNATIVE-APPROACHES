...
def _engineer_features(df: pd.DataFrame, ret_cols: list) -> pd.DataFrame:
    ...
    for col in ret_cols:
        ...
        # Use min_periods to avoid all NaN at start of series
        feat[f"{col}_vol5"] = r.rolling(5, min_periods=1).std()
        feat[f"{col}_vol21"] = r.rolling(21, min_periods=5).std()
        feat[f"{col}_mom5"] = r.rolling(5, min_periods=1).sum()
        feat[f"{col}_mom21"] = r.rolling(21, min_periods=5).sum()
    ...
    for col in MACRO_COLS:
        if col not in df.columns:
            continue
        s = df[col]
        # Z-score over rolling 252-day window, but allow partial windows (min 63 days)
    ...
    if "TLT_Ret" in df.columns and "AGG_Ret" in df.columns:
        feat["TLT_AGG_spread_mom5"] = (
            df["TLT_Ret"].rolling(5, min_periods=1).sum() - 
            df["AGG_Ret"].rolling(5, min_periods=1).sum()
        )
